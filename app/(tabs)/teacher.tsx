...
export default function TeacherScreen() {
  ...
  if (!profile || !user || (profile.role !== 'teacher' && profile.role !== 'admin')) {
    return (
      <View style={[styles.container, styles.accessDenied]}>
        <Text style={styles.title}>Teacher access required</Text>
        <Text style={styles.subtitle}>Your account does not have permission to create events.</Text>
      </View>
    );
  }
  ...
  const handleCreateEvent = async () => {
    ...
    try {
      const createdEvent = await createEvent({ ...event, created_by: user.id });
      setMessage('Event saved! Scan the QR with the Scan tab to test it.');
      setPayload(
        JSON.stringify({
          v: 1,
          event: createdEvent.event_code,
